...
import DrawingCanvas, { BitboardDemo } from "./DrawingCanvas";

// ============================================================
// BITMAP FORMAT DEFINITIONS
// ============================================================
const FORMATS = {
  banner: { width: 64, height: 16 },
  square: { width: 32, height: 32 },
  wide:   { width: 96, height: 12 },
...
function BitCanvas({ bitmap }) {
  ...
  useEffect(() => {
    ...
    const s = bitmap.scale ?? Math.max(4, Math.min(Math.floor(560 / w), Math.floor(460 / h)));
...
function CreateModal({ onClose, onPost }) {
  ...
  const [drawnBitmap, setDrawnBitmap] = useState(null);

  const F = FORMATS[fmt] || FORMATS.banner;

  const handlePost = () => {
    const pixels = drawnBitmap?.pixels ?? new Array(F.width * F.height).fill(0);
    onPost({ username: username.trim() || "anon", width: F.width, height: F.height, pixels });
    onClose();
  };
          ...
          <div>
            ...
            <input
              ...
              style={{
                ...
                border: "2px solid #1a1a1a", outline: "none",
          ...
          <div>
            ...
            <div className="fmt-grid">
              {FMT_OPTS.map(f => (
                ...
                >
                  <span className="fmt-icon" style={{ width: f.aw, height: f.ah }} />
                  {f.label}
    ...
    <>
      ...
      <div className="app">
        ...
        <div className="main">
          ...
          {activeNav === "draw"
            ? <div style={{ flex: 1, overflowY: "auto" }}><BitboardDemo /></div>
